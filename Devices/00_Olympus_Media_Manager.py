...
# Company:     -
# File:        00_Olympus_Media_Manager.py
# created:     31.01.2024 14:17
# edited:      
#              
...
def rename_files():
    ...
    subprocess.run(["exiftool", "-ext", "jpg", "-FileName<DateTimeOriginal",
                   "-d", "CAM_%Y-%m-%d_%H%M%S%%-c.%%le", jpg_folder])
    subprocess.run(["exiftool", "-ext", "jpg", "-FileName<DateTimeOriginal",
                   "-d", "CAM_%Y-%m-%d_%H%M%S%%-c.%%le", converted_jpg_folder])
    subprocess.run(["exiftool", "-ext", "png", "-FileName<DateTimeOriginal",
                   "-d", "CAM_%Y-%m-%d_%H%M%S%%-c.%%le", screenshot_folder])
    subprocess.run(["exiftool", "-ext", "jpg", "-FileName<DateTimeOriginal",
                   "-d", "CAM_%Y-%m-%d_%H%M%S%%-c_edit.%%le", edit_folder])
    subprocess.run(["exiftool", "-ee", "-api", "QuickTimeUTC", "-ext", "mov",
                   "-d", "CAM_%Y-%m-%d_%H%M%S%%-c.%%le", "-filename<CreateDate", vids_folder])
    subprocess.run(["exiftool", "-ext", "orf", "-FileName<DateTimeOriginal",
                   "-d", "CAM_%Y-%m-%d_%H%M%S%%-c.%%le", raw_folder])
